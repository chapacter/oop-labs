...
public class SearchJdbcFunctionDao implements SearchableDao<FunctionDto> {
    private static final Logger logger = LoggerFactory.getLogger(SearchJdbcFunctionDao.class);
    ...
    public SearchResult<FunctionDto> multiFieldSearch(List<SearchCriteria> criteriaList) {
        ...
        long duration = System.currentTimeMillis() - startTime;
        logger.info("Множественный поиск завершен: {} уникальных результатов из {}, время: {} мс", uniqueResults.size(), totalCount, duration);

        return new SearchResult<>(uniqueResults, uniqueResults.size(), 1, uniqueResults.size());
    }

    private void buildWhereClause(SearchCriteria criteria, StringBuilder sql, StringBuilder countSql, List<Object> parameters, List<Object> countParameters) {
    ...
    private String buildInClause(String field, List<?> values, List<Object> parameters, List<Object> countParameters) {
    ...
    private FunctionDto mapResultSetToFunction(ResultSet rs) throws SQLException {
        FunctionDto function = new FunctionDto(
                ...
                rs.getLong("user_id"),
                rs.getString("name"),
                rs.getObject("format", Integer.class),
                rs.getString("func_result")
        );
