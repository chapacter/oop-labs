...
import {
  ...
  Paper, CircularProgress, Tooltip, Divider, FormGroup, FormControlLabel, Switch
} from '@mui/material';
import {
...
const FunctionEditor: React.FC<FunctionEditorProps> = ({ mathFunctionMode = false }) => {
  ...
  const loadFunctionData = async (functionId: number) => {
    try {
      ...
      setFuncResult(funcData.funcResult);
    ...
    try {
      ...
      if (mathFunctionMode && !isEditing) {
        ...
        const newFunction = await functionService.createFromMathFunction(
          ...
          userId,
          funcResult || `Результат функции ${mathFunctionType}`,
          mathFunctionType,
          intervalStart,
          intervalEnd,
          pointsCount
        ...
        await functionService.updateFunction(Number(id), name, funcResult);
        ...
        const newFunction = await functionService.createFunctionFromArray({
          ...
          userId: userId,
          funcResult: funcResult || 'Результат функции',
          points: sortedPoints
        });
      ...
      toast.error('Не удалось сохранить функцию. Проверьте введенные данные.');
    } finally {
      setLoading(false);
    }
  };
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                          <TextField
                            ...
                            value={funcResult}
                            onChange={(e) => setFuncResult(e.target.value)}
                            required
                          />
                        </Grid>
                      </Grid>
